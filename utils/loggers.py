...
def print_accuracy(mean_acc: np.ndarray, batch_number: int, heldout_acc: float, heldout_angle:float, memorizaton:float,
                        setting: str) -> None:
    """
    Prints the mean accuracy on stderr.
    :param mean_acc: mean accuracy value
    :param batch_number: number of seen batches
    :param setting: the setting of the benchmark
    """
    if setting == 'domain-il':
        mean_acc, _ = mean_acc
        print('\nAvg Acc across all domains for {} batches seen: {} and Acc on heldout: {} at angle {} and Mem Acc {}%'.format(
            batch_number, round(mean_acc, 2), round(heldout_acc,2), round(heldout_angle,2), memorizaton), file=sys.stderr)
    else:
        mean_acc_class_il, mean_acc_task_il = mean_acc
        print('\nAccuracy for {} task(s): \t [Class-IL]: {} %'
              ' \t [Task-IL]: {} %\n'.format(batch_number, round(
