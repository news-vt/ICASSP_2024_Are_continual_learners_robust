...
class ContinualModel(nn.Module):
    ...
    def meta_observe(self, *args, **kwargs): 
        """
        I guess it calls self.observe and records it in wandb
        """
    ...
    def meta_memorization(self, *args, **kwargs):
        """
        Compute memorization performnace using the current memeory 
        """
        if 'wandb' in sys.modules and not self.args.nowand:
            pl = persistent_locals(self.memorization)
            ret = pl(*args, **kwargs)
            self.autolog_wandb(pl.locals)
        else:
            raise NotImplementedError
